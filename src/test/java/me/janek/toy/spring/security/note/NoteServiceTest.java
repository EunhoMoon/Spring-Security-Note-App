...
import me.janek.note.note.NoteRepository;
...
class NoteServiceTest {
    ...
    void findByUser_유저가_게시물조회() {
        ...
        // note1 = title2
        BDDAssertions.then(note1.getUser().getUsername()).isEqualTo("username");
        then(note1.getTitle()).isEqualTo("title2"); // 가장 늦게 insert된 데이터가 먼저 나와야합니다.
        then(note1.getContent()).isEqualTo("content2");
        // note2 = title1
        BDDAssertions.then(note2.getUser().getUsername()).isEqualTo("username");
    ...
    void findByUser_어드민이_조회() {
        ...
        // note1 = title3
        BDDAssertions.then(note1.getUser().getUsername()).isEqualTo("username2");
        ...
        // note3 = title1
        BDDAssertions.then(note3.getUser().getUsername()).isEqualTo("username");
